...
if (isset ( $ac ) && $ac == 'del') {
	...
	$where = " 1 = 1 "; //默认查询条件
	intval ( $w ['email_a_id'] ) and $where .= " and email_a_id = " . intval ( $w ['email_a_id'] ) . ""; //搜索认证编号
...
if($action){
	...
	if($action=='pass'){
		...
		//updaet autho_itme table
		$sql = sprintf("update %switkey_auth_record set `auth_status` = 1 where auth_code='email' and uid = '%d' ",TABLEPRE,$euid);
	...
	kekezu::admin_show_msg($_lang['operate_success'],'index.php?do=auth&view=list&auth_code=email',3,'','success');
}
