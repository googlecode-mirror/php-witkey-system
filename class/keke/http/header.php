...
class Keke_HTTP_Header extends ArrayObject {
	...
	public static function accept_quality(array $parts)
	{
		...
		foreach ($parts_keys as $key)
		{
			...
			{
				$parsed[$value] = (float) Keke_HTTP_Header::DEFAULT_QUALITY;
			}
			else
			{
				$quality = $quality[2];
	...
	public static function parse_accept_header($accepts = NULL)
	{
		...
		if ($accepts === NULL)
			return array('*' => array('*' => (float) Keke_HTTP_Header::DEFAULT_QUALITY));

		// Parse the accept header qualities
		$accepts = Keke_HTTP_Header::accept_quality($accepts);
	...
	public static function parse_charset_header($charset = NULL)
	{
		if ($charset === NULL)
		{
			return array('*' => (float) Keke_HTTP_Header::DEFAULT_QUALITY);
		}

		return Keke_HTTP_Header::accept_quality(explode(',', (string) $charset));
	}
	...
	public static function parse_encoding_header($encoding = NULL)
	{
		...
		elseif ($encoding === '')
		{
			return array('identity' => (float) Keke_HTTP_Header::DEFAULT_QUALITY);
		}
		else
		{
			return Keke_HTTP_Header::accept_quality(explode(',', (string) $encoding));
		}
	}
	...
	public static function parse_language_header($language = NULL)
	{
		...
		$language = Keke_HTTP_Header::accept_quality(explode(',', (string) $language));
	 ...
	 *     $response->headers('Cache-Control', Keke_HTTP_Header::create_cache_control($cache_control);
	 *
	 ...
	 *     if ($cache_control = Keke_HTTP_Header::parse_cache_control($response->headers('cache-control')))
	 *     {
	...
	/**
	 * Parses a HTTP Message header line and applies it to this Keke_HTTP_Header
	 * 
	...
	public function accepts_at_quality($type, $explicit = FALSE)
	{
		...
		if ($this->_accept_content === NULL)
		{
			...
			$this->_accept_content = Keke_HTTP_Header::parse_accept_header($accept);
		}
	...
	public function accepts_charset_at_quality($charset)
	{
		if ($this->_accept_charset === NULL)
		{
			if ($this->offsetExists('Accept-Charset'))
			{
				...
				$this->_accept_charset = Keke_HTTP_Header::parse_charset_header($charset_header);
			}
			else
			{
				$this->_accept_charset = Keke_HTTP_Header::parse_charset_header(NULL);
			}
		}
	...
	public function accepts_encoding_at_quality($encoding, $explicit = FALSE)
	{
		if ($this->_accept_encoding === NULL)
		{
			...
			$this->_accept_encoding = Keke_HTTP_Header::parse_encoding_header($encoding_header);
		}
		...
		if ($explicit === FALSE)
		{
			...
			{
				return (float) Keke_HTTP_Header::DEFAULT_QUALITY;
			}
		}
	...
	public function accepts_language_at_quality($language, $explicit = FALSE)
	{
		if ($this->_accept_language === NULL)
		{
			...
			$this->_accept_language = Keke_HTTP_Header::parse_language_header($language_header);
		}
