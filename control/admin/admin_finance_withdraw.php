...
if (isset ( $ac )) { //处理财务清单申请
	switch ($ac) {
		...
		case 'del' :
			if ($withdraw_info) {
				...
				Keke::notify_user ( $_lang['fail_and_check_you_account'], $uid, $username );
				if ($space_info ['email']) {
					Keke::send_mail ( $space_info ['email'], $_lang['audit_withdraw_apply_fail'], $_lang['not_pass_and_check_you_account'] );
				}
		...
		switch ($sbt_action) {
			case $_lang['mulit_delete']: //批量删除
				...
				foreach ( $nodraw_arr as $v ) {
					...
					keke_finance_class::cash_in ( $uid, $withdraw_cash, 0, 'withdraw_fail' );
					Keke::notify_user ( $_lang['audit_withdraw_fail'], $_lang['not_pass_and_check_you_account'], $uid, $username );
					if ($space_info ['email']) {
						Keke::send_mail ( $space_info ['email'], $_lang['audit_withdraw_fail'], $_lang['not_pass_and_check_you_account'] );
					}
				}
				//审核通过的直接删除
				$res = $withdraw_obj->del_keke_witkey_withdraw ();
			...
			case $_lang['mulit_review']: //批量审核
				...
				foreach ( $withdraw_arr as $withdraw_info ) {
					$space_info = Keke::get_user_info ( $withdraw_info ['uid'] );
	...
	//Keke::update_score_value ( $withdraw_info ['uid'], 'withdraw', 2 );
				//Keke::feed_add ( '<a href="index.php?do=space&member_id=' . $space_info ['uid'] . '" target="_blank">' . $withdraw_info ['username'] . '</a>成功提现了' . $withdraw_info ['withdraw_cash'] . '元', $user_space_info ['uid'], $user_space_info ['username'], 'withdraw' );
				}
