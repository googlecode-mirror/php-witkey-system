...
if (isset ( $ac )) {
	if ($item_id && $ac = 'del') {
		...
		Keke::admin_system_log ($_lang['delete_prom_material']. "$item_id" );
		Keke::admin_show_msg ( $res ? $_lang['delete_prom_material_success'] : $_lang['operate_fail_please_choose_again'], $url,3,'', $res?'success':'warning' );
	} else {
...
if (isset ( $sbt_action )) { //批量删除
	...
	if (count ( $ckb_string )) {
		...
		$res = $prom_item_obj->del_keke_witkey_prom_item ();
		Keke::admin_system_log ($_lang['delete_prom_material']."$ids" );
		Keke::admin_show_msg ( $res ? $_lang['mulit_operate_success'] : $_lang['mulit_operate_fail_please_again'], $url,3,'', $res?'success':'warning');
	} else
