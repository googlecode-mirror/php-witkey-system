...
class Control_admin_login extends Controller {
	...
	function action_login() {
		...
		//$allow_times = $this->admin_obj->times_limit ( $_POST['allow_time'] ); // 允许登录尝试次数
		if ($is_submit) {
			$user_name = Keke::utftogbk ( $_POST ['user_name'] );
			$this->admin_obj->admin_login ( $_POST ['user_name'], $_POST ['pass_word'], $_POST['allow_num'], $token );
			die ();
		}
	}
}
