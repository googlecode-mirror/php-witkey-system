	...
	function action_charge(){
		...
		if($_POST['user']){
			...
			if ($_POST['cash_type']==0&&$_POST['cash']>$info['balance']){
				keke::show_msg("系统提示","index.php/admin/user_add/charge","扣除的现金超出了可用现金!!!","warning");
			...
			if ($_POST['credit_type']==0&&$_POST['credit']>$info['credit']){
				keke::show_msg("系统提示","index.php/admin/user_add/charge","扣除的元宝超出了可用元宝!!!","warning");
			...
			if(!$_POST['cash']&&!$_POST['credit']){
				keke::show_msg("系统提示","index.php/admin/user_add/charge","充值或者扣除金额不得为0或者空!!!","warning");
			}
			//现金如果为添加则为收入，反之为支出
			/**
			 * 财务表加入管理员操作记录
