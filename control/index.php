 ...
 * 首页最近二周任务中标额统计
 */
$two_week_task_in = db_factory::query ( sprintf ( " select sum(fina_cash) cash,WEEK(date(from_unixtime(fina_time)),1) week from %switkey_finance where fina_action='task_bid' and fina_type='in' group by week limit 0,2", TABLEPRE ), 1, 600 ); // 任务
 ...
 * 首页最近二周任务发布量统计
 */
$two_week_task_count = db_factory::query ( sprintf ( " select count(task_id) count,WEEK(date(from_unixtime(start_time)),1) week from %switkey_task group by week limit 0,2", TABLEPRE ), 1, 600 ); // 数量统计
 ...
 * 首页最近二周服务少收额统计
 */
$two_week_service_in = db_factory::query ( sprintf ( " select sum(fina_cash) cash,WEEK(date(from_unixtime(fina_time)),1) week from %switkey_finance where fina_action='sale_service' and fina_type='in' group by week limit 0,2", TABLEPRE ), 1, 600 ); // 商城
 ...
 * 首页最近二周服务发布量统计
 */
$two_week_service_count = db_factory::query ( sprintf ( " select count(service_id) count,WEEK(date(from_unixtime(on_time)),1) week from %switkey_service where service_status='2' group by week limit 0,2", TABLEPRE ), 1, 600 ); // 数量统计
 ...
 * 首页最近二周注册用户统计
 */
$two_week_register = db_factory::query ( sprintf ( " select count(uid) count,WEEK(date(from_unixtime(reg_time)),1) week from %switkey_space where status!='2' group by week limit 0,2", TABLEPRE ), 1, 600 ); // 注册用户
 ...
 * 首页最近二周认证用户统计
 */
$two_week_auth = db_factory::query ( sprintf ( " select count(record_id) count,WEEK(date(from_unixtime(end_time)),1) week from %switkey_auth_record where auth_status='1' group by week limit 0,2", TABLEPRE ), 1, 600 ); // 认证用户
 ...
 * 收入排行
 */
$income_rank = db_factory::query ( sprintf ( " select sum(fina_cash) as cash,uid,username from %switkey_finance where fina_type='in' and fina_action!='task_fail' group by uid order by cash desc limit 0,5 ", TABLEPRE ), 1, 600 ); // 收入排行
if (isset ( $ajax )) {
	switch ($ajax) {
		case "task" :
			/**
