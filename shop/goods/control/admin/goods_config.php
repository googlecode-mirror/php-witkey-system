...
if (isset ( $sbt_edit )) {
	...
	switch ($op){
		case 'config':						
			...
			Keke::admin_show_msg ( $_lang['goods_basic_config_update_success'], $url,10,'','success' );			
			break;
		case 'control':					
			is_array($cont) and $res = keke_task_config::set_task_ext_config($model_id,$cont);
			$res and Keke::admin_show_msg($_lang['goods_flow_config_update_success'],$url,10,'','success') or Keke::admin_show_msg($_lang['goods_flow_config_update_fail'],$url,10,'','warning');						
			break;
		case 'rule':
			if ($fds ['allow_times']){
				...
				Keke::admin_show_msg ( $_lang['goods_permissions_config_update_success'], "index.php?do=model&model_id=$model_id&view=config&op=rule",3,'','success');
			}
			break;
	}
}
