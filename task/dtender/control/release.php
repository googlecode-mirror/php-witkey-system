...
$release_obj = dtender_release_class::get_instance ( $model_id,$pub_mode);

$payitem_arr = keke_payitem_class::get_payitem_info('employer','dtender'); //获取该任务所有的增值服务  
...
$cash_cove = $release_obj->_cash_cove;//招标区间
...
switch ($r_step) { //任务发布步骤
	...
	switch ($ajax) {
		...
		if (kekezu::submitcheck($formhash)) {
			...
			$total_cash = $release_obj->get_total_cash ( $task_config['deposit'] ); //任务总金额
