...
$work_prize = $task_obj->get_work_prize();//稿件奖项
$task_prize_arr = $task_obj->get_task_prize();//任务奖项数组
$task_obj->task_tg_timeout();//判断投稿期是否结束
$task_obj->task_xg_timeout();//判断选稿期是否结束
...
switch ($view) {
	case "work" :
		...
		$work_arr = $task_obj->get_work_info ($w, " work_id desc ", $p ); //稿件信息
