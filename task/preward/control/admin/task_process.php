	...
	switch ($type) {
		case 'report':
			...
			if($op_result['action']=='pass'){
				$res  and Keke::admin_show_msg($_lang['operate_notice'],$url,3,$action_arr[$type]['1'].$_lang['operation_completed'],'success') or Keke::admin_show_msg($_lang['operate_notice'],$url,3,$action_arr[$type]['1'].$_lang['operate_fail'],'warning');
			}else{
