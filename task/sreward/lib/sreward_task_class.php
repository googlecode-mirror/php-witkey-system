		...
		if ($this->_task_status == 3 && $this->_task_info ['end_time'] < time ()) { 
			...
			if ($this->_task_info ['work_num'] > 0) { 
				...
				if ($rw_count == '1') { 
					...
					db_factory::execute ( sprintf ( " update %switkey_task set is_auto_bid='1' where task_id='%d'", TABLEPRE, $this->_task_id ) );
					//发送站内短信给雇主
					$v_arr = array ($_lang ['username'] => '$this->_gusername', $_lang ['model_name'] => $this->_model_name, $_lang ['task_id'] => $this->_task_id, $_lang ['task_title'] => $this->_task_title, $_lang ['reason'] => $_lang ['xg_timeout'], $_lang ['time'] => date ( 'Y-m-d,H:i:s', time () ), 'next' => $_lang ['gs'] );
