<?php
keke_lang_class::load_lang_class('taobao_report_class');
class taobao_report_class extends keke_report_class {
	
	public static function get_instance($report_id, $report_info = null, $obj_info = null,$user_info=null,$to_userinfo=null) {
		static $obj = null;
		if ($obj == null) {
			$obj = new taobao_report_class ( $report_id, $report_info, $obj_info,$user_info,$to_userinfo);
		}
		return $obj;
	}
		...
		switch ($op_result ['action']) {
			case "pass" :
				if ($this->_process_can ['sharing']) { //可以分配
					...
					$gz_get = floatval (keke_curren_class::convert($op_result['gz_get'],0,true)); //雇主分得佣金
					$wk_get = floatval (keke_curren_class::convert($op_result['wk_get'],0,true)); //威客分得佣金
