...
//任务状态
$task_status = wbdj_task_class::get_task_status ();
...
$url_str = "index.php?do=model&model_id=9&view=list&w[task_id]=$w[task_id]&w[wb_platform]=$w[wb_platform]&ord[0]=$ord[0]&ord[1]=$ord[1]&page=$page&page_size=$page_size";

$sql_count = "select count(a.task_id) from ".TABLEPRE."witkey_task_wbdj b left join ".TABLEPRE."witkey_task a on a.task_id = b.task_id where ".$wh;
...
$sql = "select a.*,b.* from ".TABLEPRE."witkey_task_wbdj b left join ".TABLEPRE."witkey_task a on a.task_id = b.task_id where".$wh;
