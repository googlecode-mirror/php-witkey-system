		...
		if ($ac && $work_id) {
			switch ($ac) {
				case 'del' : //É¾³ý
					...
					$res_wbdj = db_factory::execute(sprintf('delete from %switkey_task_wbdj_work where work_id=%d',TABLEPRE,intval($work_id)));
					$res = ($res&&$res_wbdj);
					if ($res) {
			...
			$sql = sprintf("select w_basic.*, w_wbdj.`wb_type`,w_wbdj.`wb_url`,w_wbdj.`click_num` from %switkey_task_work w_basic left join %switkey_task_wbdj_work w_wbdj on w_basic.`task_id`=w_wbdj.`task_id` where w_basic.task_id=%d",TABLEPRE,TABLEPRE,$task_id);
			$tmp = db_factory::query($sql);
			$pages = $page_obj->page_by_arr($tmp, $page_size, $page, $url);
			$list = $pages ['data'];
			$satus_arr = wbdj_task_class::get_work_status ();
