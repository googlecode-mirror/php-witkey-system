...
class XML{
	...
	static function  XML_unserialize($xml){
		$xml_parser = new XML();
	...
	static function  XML_serialize($data, $level = 0, $prior_key = NULL){
		...
		while(list($key, $value) = each($data))
			...
			XML::XML_serialize($value, $level, $key);
		}else{
			...
			else echo ">\n",XML::XML_serialize($value, $level+1),str_repeat("\t", $level),"</$tag>\n";
		}
		reset($data);
	...
	static function count_numeric_items($array){
...
$x = file_get_contents('d.xml');


$arr = XML::XML_unserialize($x);

var_dump($arr);

$arr['shipin']['ad attr']['name'] = 'text.flv';
$arr['shipin']['ad attr']['url'] = 'http://www.kekezu.com';
$c  = Keke::gbktoutf('一生一世');
$arr['shipin']['ad'] = $c;


$xx = XML::XML_serialize($arr);

var_dump($xx);

file_put_contents('d.xml', $xx);
