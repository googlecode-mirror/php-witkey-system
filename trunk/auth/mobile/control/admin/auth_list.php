...
if (isset ( $ac )) {
	switch ($ac) {
		case "pass" : //单条通过认证操作
			...
			Keke::admin_system_log($obj.$_lang['mobile_auth_pass']);
		
			$auth_obj->review_auth ( $record_id, 'pass' );
			break;
		case "not_pass" : //单条不通过认证操作
			Keke::admin_system_log($obj.$_lang['mobile_auth_nopass']);
			$auth_obj->review_auth ( $record_id, 'not_pass' );
			break;
			;
		case 'del' : //单条删除认证申请
			Keke::admin_system_log($obj.$_lang['mobile_auth_delete']);
	...
	switch ($sbt_action) {
		case $_lang['mulit_delete']: //批量删除
			Keke::admin_system_log($_lang['mulit_delete_mobile']);
			$auth_obj->del_auth ( $keyids );
			break;
			;
		case $_lang['mulit_pass']: //批量审核
			Keke::admin_system_log($_lang['mulit_pass_mobile']);
			$auth_obj->review_auth ( $keyids, 'pass' );
			break;
			;
		case $_lang['mulit_nopass']: //批量不审核
			Keke::admin_system_log($_lang['mulit_nopass_mobile']);
