...
class Keke_HTTP_Cache {
	...
	public function cache_key_callback($callback = NULL)
	{
		...
		if ( ! is_callable($callback))
			throw new Keke_exception('cache_key_callback must be callable!');
	...
	public function create_cache_key(Request $request, $callback = FALSE)
	{
		if (is_callable($callback))
			...
			return Keke_HTTP_Cache::basic_cache_key_generator($request);
	}
	...
	public function set_cache(Response $response)
	{
		...
		if ($expires = $headers['expires'] AND ! isset($cache_control['max-age']))
		{
