...
class Keke_Route {
	 ...
	 */
	public static $localhosts = array (
			FALSE,
			'',
			'local',
			'localhost' 
	...
	public static function set($name, $uri = NULL, $regex = NULL) {
		return Route::$_routes [$name] = new Keke_Route ( $uri, $regex );
	}
	...
	public static function get($name) {
		if (! isset ( Route::$_routes [$name] )) {
			throw new Keke_exception ( 'The requested route does not exist: :route', array (
					':route' => $name 
			) );
		}
	 ...
	 * If your routes will remain the same for
	 ...
	 *        	append, rather than replace, cached routes when loading
	...
	public static function cache($save = FALSE, $append = FALSE) {
		if ($save === TRUE) {
			try {
				...
				// We most likely have a lambda in a route, which cannot be
				// cached
				throw new Keke_exception ( 'One or more routes could not be cached (:message)', array (
						':message' => $e->getMessage () 
				), 0, $e );
			}
		} else {
			if ($routes = Keke::cache ( 'Route::cache()' )) {
				if ($append) {
	 ...
	 *        	protocol string or boolean, adds protocol and domain
	 * @return string
	 * @since 3.0.7
	 * @uses URL::site
	 */
	 ...
	 *        	callback string, array, or closure
	 * @return $this
	 */
	public function filter($callback) {
		if (! is_callable ( $callback )) {
			throw new Keke_exception ( 'Invalid Route::callback specified' );
		}
	 ...
	 * A successful match will return
	...
	public function matches($uri) {
		...
		if ($this->_filters) {
			foreach ( $this->_filters as $callback ) {
				...
				$return = call_user_func ( $callback, $this, $params, Request::factory() );
