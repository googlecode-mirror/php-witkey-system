...
if ($submit) {
	...
	$Keke->_cache_obj->del ( 'tag_list_cache' );
	if ($res) {
		Keke::admin_show_msg ($_lang['tag_change_success'], "index.php?do=tpl&view=taglist&tag_type=$tag_type&type=$type",3,'','success' );
	} else {
		Keke::admin_show_msg ($_lang['tag_change_fail'], "index.php?do=tpl&view=edit_tag&tagid=$tagid&tag_type=$tag_type&type=$type",3,'','warning' );
	}
