...
{ 
	 ...
	 * @return array
	 */
	function queryid( $mid, $type = 1, $is_batch = 0, $inbox = 0, $isBase62 = 1)
	{
		$params = array();
		$params['mid'] = $mid;
...
// 		$params['inbox'] = intval($inbox);
		...
		$result = $this->oauth->get('http://api.t.sina.com.cn/queryid.json', $params);
// 		var_dump($result,$_SESSION['auth_sina']['last_key']);die();
		return $result;
	}
...
//         	var_dump($response);
        if ($this->format === 'json' && $this->decode_json) { 
        	$response = $this->inttostring($response);
    ...
    function inttostring($json){
    $json = preg_replace('/"id":(\d+)[10,16]/s', '"id":"\\1",', $json);
	$json = preg_replace('/"user":{"id":(\d+)[10,15]/s', '"user":{"id":"\\1",', $json);
    return $json;	
    }
    /** 
...
//         if ($url=='http://api.t.sina.com.cn/statuses/queryid.json'){
//         	$url = 'http://api.weibo.com/2/statuses/queryid.json';
//         }
//         echo $url;
