...
class sohu_oauth_client_class extends base_client_class{
	...
	function __construct($app_key,$app_secret){
		...
		$this->_souhu_weibo_oauth = new SohuOAuth($app_key, $app_secret);
	}
	...
	function get_auth_url($callback){
		...
		$request_token = $o->getRequestToken(OAUTH_CALLBACK);
		$_SESSION['auth_sohu']['oauth_token'] = $token = $request_token['oauth_token'];
		$_SESSION['auth_sohu']['oauth_token_secret'] = $request_token['oauth_token_secret'];
		switch ($o->http_code) {
		  case 200:
		    /* 获取用户认证地址，并且重定向到SOHU */
		    $url = $o->getAuthorizeUrl1($token, $callback);
		    break;
		  default:
	...
	function get_access_token(){
		return $_SESSION['auth_sohu']['last_key'];
	}
	...
	function create_access_token($oauth_verifier=false){
		...
		if (isset($_REQUEST['oauth_token']) && $_SESSION['auth_sohu']['oauth_token'] !== $_REQUEST['oauth_token']) 
		{
			$this->set_error('access_token不存在或者已过期');
			return false;
		}
		$o->__construct($this->_app_key,$this->_app_secret,$_SESSION['auth_sohu']['oauth_token'] ,$_SESSION['auth_sohu']['oauth_token_secret'] );
		$access_token = $o->getAccessToken($oauth_verifier);
		$_SESSION['auth_sohu']['last_key'] = $access_token;
		
		unset($_SESSION['auth_sohu']['oauth_token']);
		unset($_SESSION['auth_sohu']['oauth_token_secret']);
		
		if (200 == $o->http_code) {
		  /* The user has been verified and the access tokens can be saved for future use */
		  $_SESSION['auth_sohu']['status'] = 'verified';
		  return true;
		} else {
		  return false;
		}
	...
	private function init_client(){
		...
		$this->_souhu_weibo_client = new MBApiClient($this->_app_key,$this->_app_secret, $_SESSION['auth_sohu']['last_key']['oauth_token'] ,$_SESSION['auth_sohu']['last_key']['oauth_token_secret']);
		}
	...
	function wb_data_format($data){
		...
		$r['img'] = $data['small_pic'];
		$r['url']="http://t.sohu.com/m/{$data['id']}";var_dump($r);die();
		return $r;
	}
