...
if (isset ( $sbt_edit )) {
	...
	switch ($op){
		...
		case 'control':				
			...
			$res and kekezu::admin_show_msg($_lang['goods_flow_config_update_success'],$url,3,'','success') or kekezu::admin_show_msg($_lang['goods_flow_config_update_fail'],$url,3,'','warning');						
			break;
		case 'rule':
			if ($fds ['allow_times']){
