	...
	public function work_choose($bid_id, $to_status, $url = '', $output = 'normal', $trust_response = false) {
		...
		if ($this->set_bid_status ( $bid_id, $to_status )) {
			...
			if ($to_status == 4) { 
				...
				$v = array ($_lang ['task_id'] => $this->_task_id, $_lang ['task_title'] => $url, $_lang ['bid_cash'] => $bid_info ['quote'] );
                //通知威客
			...
			if ($this->plan_cash ( $plan_id, $title_url )) {
				if (intval ( $plan_step ) == $size) {
					...
					if ($this->_task_info ['task_union']==2) {
						...
						$u->task_close ( array ('r_task_id' => $u->_r_task_id, 'indetify' => 1,'bid_uid'=>$this->_bid_info ['uid']) );
					}
	...
	public function task_tb_timeout() {
		...
		if ($this->_task_status == 2 && $this->_task_info ['sub_time'] < time ()) { 
			...
			if ($work_info) {
				...
				//发消息
				$v_arr = array ($_lang ['username'] => $this->_bid_info ['username'], $_lang ['model_name'] => $this->_model_name, $_lang ['task_id'] => $this->_task_id, $_lang ['task_title'] => $this->_task_title, $_lang ['tb'] => $_lang ['tendering'], 'time' => date ( 'Y-m-d,H:i:s', time () ), 'next' => $_lang ['choosing_tender'] );
