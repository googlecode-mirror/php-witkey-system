			...
			if (time () > intval ( $this->_task_info ['sub_time'] )) {
				...
				if (intval ( $this->_task_info ['work_num'] ) > 0) { 
					...
					if ($this->set_task_status ( 3 )) {
						//发消息 
						$arr = array ($_lang ['username'] => $this->_gusername, $_lang ["model_name"] => $this->_model_name, $_lang ["task_id"] => $this->_task_id, $_lang ["task_title"] => $this->_task_title, $_lang ["tb"] => $_lang ['hand_work'], "next" => $_lang ['choose_work'] );
