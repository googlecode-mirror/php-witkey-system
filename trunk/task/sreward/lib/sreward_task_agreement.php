...
class sreward_task_agreement extends keke_task_agreement {
	...
	public function process_can() {
		...
		$buyer_status = $this->_buyer_status; //买家状态
		$seller_status = $this->_seller_status; //卖家状态
		$buyer_uid = $this->_buyer_uid; //买家uid
		$seller_uid = $this->_seller_uid; //卖家uid 
		$process_arr = array ();
		if ($uid == $buyer_uid || $uid == $seller_uid) {
			switch ($agree_status) {
				...
				case "2" : //进入实际流程
					...
					if ($uid == $seller_uid && $seller_status == '2') { //卖家状态为2,可以上传附件
						$process_arr ['upload'] = true;
					}
					if ($uid == $buyer_uid && $seller_status == '3') { //卖家已上传、买家可确认下载
						$process_arr ['confirm'] = true;
					}
					if ($uid == $seller_uid || ($uid == $buyer_uid && $buyer_status == '3')) { //卖家或者已确认附件的买家
	...
	/**
	 * 协议阶段进入权限判断
	 * @param int $user_type 用户角色 1=>威客(卖家),2=>雇主(买家)
	 * @return step  返回当前阶段值
	 */
		...
		switch ($agree_status) {
			case "1" :
				if ($buyer_status == '1' && $seller_status == '1') { //双方均未签署
					$step = 'step1';
				} elseif ($buyer_status == '1' && $seller_status == '2') { //卖家(威客)签署
					$user_type == '1' and $step = 'step2' or $step = 'step1';
				} elseif ($buyer_status == '2' && $seller_status == '1') { //买家(雇主)签署
