...
$release_obj = wbdj_release_class::get_instance ( $model_id,$pub_mode);
...
$fans_arr = $task_config['fans_rule'];

$min =time()+ 24*3600*$task_config['min_day'];
$min = date("Y-m-d",$min); 
$max_day = $kekezu->get_show_day ( $task_cash,9 ); 
$payitem_arr = keke_payitem_class::get_payitem_info('employer','wbdj'); //获取该任务所有的增值服务  
...
switch ($r_step) { //任务发布步骤
	...
	switch ($ajax) {
		...
		if (kekezu::submitcheck($formhash)) {
			...
			$release_obj->save_task_wbdj($task_id);//存储转发信息
